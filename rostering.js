(async()=>{
  ...
  function render(){
    ...
    const confirmRestOverride=(staffId,violation)=>confirm(`${restRuleMessage(staffId,violation)}\n\nThis assignment would override the five-day work rule. Do you still want to place the employee on this date?`);
    const assignStaff=async(cell,id,department)=>{if(!id)return;if(department!==cell.dataset.department)return alert('This staff member belongs to a different department.');const duplicate=assignments.some(a=>String(a.roster_row_id)===String(cell.dataset.row)&&a.assignment_date===cell.dataset.date&&String(a.staff_member_id)===String(id));if(duplicate)return alert('This employee is already assigned to this shift.');const violation=restRuleViolation(id,cell.dataset.date);if(violation&&!confirmRestOverride(id,violation))return;await DB.create('roster_assignments',{week_start_date:week,roster_row_id:cell.dataset.row,assignment_date:cell.dataset.date,staff_member_id:id});selectedStaffId='';if(violation)Shared.toast('Five-day work rule override applied.');await load()};
    const moveAssignment=async(cell,assignmentId)=>{const assignment=assignments.find(a=>String(a.id)===String(assignmentId));if(!assignment)return;if((assignment.department||assignment.staff_type)!==cell.dataset.department)return alert('This assignment belongs to a different department.');if(String(assignment.roster_row_id)===String(cell.dataset.row)&&assignment.assignment_date===cell.dataset.date)return;const duplicate=assignments.some(a=>String(a.id)!==String(assignment.id)&&String(a.roster_row_id)===String(cell.dataset.row)&&a.assignment_date===cell.dataset.date&&String(a.staff_member_id)===String(assignment.staff_member_id));if(duplicate)return alert('This employee is already assigned to the destination shift.');const violation=restRuleViolation(assignment.staff_member_id,cell.dataset.date,{excludeAssignmentId:assignment.id});if(violation&&!confirmRestOverride(assignment.staff_member_id,violation))return;await DB.update('roster_assignments',assignment.id,{roster_row_id:cell.dataset.row,assignment_date:cell.dataset.date,week_start_date:week});Shared.toast(violation?'Five-day work rule override applied.':`${assignment.staff_name||'Employee'} moved to the new shift.`);await load()};
