(async()=>{
  ...
  async function load(){[staff,allRows,assignments,allAssignments]=await Promise.all([DB.list('staff_members','status=Active'),DB.list('roster_rows'),DB.list('roster_assignments',`week_start_date=${week}`),DB.list('roster_assignments')]);rows=allRows.filter(row=>row.is_active!==false);render()}
