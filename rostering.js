(async()=>{
  ...
  $('publish').onclick=async()=>{
    ...
    const snapshots=availableDepartments.map(dept=>{
      ...
      const typeRows=allRows.filter(row=>(row.department||row.staff_type)===dept.key&&row.is_active!==false).sort((a,b)=>(a.display_order||0)-(b.display_order||0)).map(row=>({...row,staff_snapshot:departmentStaff}));
