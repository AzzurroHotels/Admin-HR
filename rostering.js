(async()=>{
  ...
  const isDayOnlyRow=row=>{
    const text=`${row?.property_name||row?.row_name||''} ${row?.shift_label||''}`.trim().toLowerCase();
    return /(?:day(?:s)?\s*off|on[\s-]*leave|requested\s*leave|annual\s*leave|sick\s*leave)/.test(text);
  };
  ...
  function render(){
    ...
    $('rb').innerHTML=visibleRows.map(r=>{const dayOnly=isDayOnlyRow(r);return `<tr class="${dayOnly?'day-only-row':''}"><td><strong>${Shared.esc(r.property_name||r.row_name)}</strong></td><td>${Shared.esc(r.shift_label||'')}${dayOnly?'':`<div class="muted">${Shared.esc(r.start_time||'')}–${Shared.esc(r.end_time||'')}</div>`}</td>${ds.map((d,dayIndex)=>{const date=iso(d),days=Array.isArray(r.active_days)?r.active_days:[0,1,2,3,4,5,6];if(!days.includes(dayIndex))return '<td class="roster-cell inactive-cell">—</td>';const a=assignments.filter(x=>String(x.roster_row_id)===String(r.id)&&x.assignment_date===date);return `<td class="roster-cell${dayOnly?' day-only-cell':''}" data-row="${r.id}" data-date="${date}" data-department="${activeDepartment}"><div class="${dayOnly?'assignment-stack':''}">${a.map(x=>{const conflict=conflictIds.has(String(x.id));return `<span class="assignment draggable-assignment${conflict?' assignment-conflict':''}" draggable="true" data-assignment="${x.id}" title="${conflict?'Schedule conflict: this employee has another overlapping shift':'Drag to move this employee to another shift'}">${Shared.esc(x.staff_name)} <button data-remove="${x.id}" aria-label="Remove">×</button></span>`}).join('')}</div></td>`}).join('')}</tr>`}).join('')||'<tr><td colspan="9">No active shifts. Use Manage Roles & Shifts to add one.</td></tr>';
    ...
    const assignStaff=async(cell,id,department)=>{if(!id)return;if(department!==cell.dataset.department)return alert('This staff member belongs to a different department.');const duplicate=assignments.some(a=>String(a.roster_row_id)===String(cell.dataset.row)&&a.assignment_date===cell.dataset.date&&String(a.staff_member_id)===String(id));if(duplicate)return alert('This employee is already assigned to this shift.');const targetRow=allRows.find(row=>String(row.id)===String(cell.dataset.row));const dayOnly=isDayOnlyRow(targetRow);const dayViolation=dayOnly?null:restRuleViolation(id,cell.dataset.date);const hourViolation=dayOnly?null:eightHourRestViolation(id,cell.dataset.date,cell.dataset.row);if(!confirmRuleOverrides(id,dayViolation,hourViolation))return;await DB.create('roster_assignments',{week_start_date:week,roster_row_id:cell.dataset.row,assignment_date:cell.dataset.date,staff_member_id:id});selectedStaffId='';if(dayViolation||hourViolation)Shared.toast(`${dayViolation&&hourViolation?'Five-day and 8-hour rest':dayViolation?'Five-day work':'8-hour rest'} rule override applied.`);await load()};
    const moveAssignment=async(cell,assignmentId)=>{const assignment=assignments.find(a=>String(a.id)===String(assignmentId));if(!assignment)return;if((assignment.department||assignment.staff_type)!==cell.dataset.department)return alert('This assignment belongs to a different department.');if(String(assignment.roster_row_id)===String(cell.dataset.row)&&assignment.assignment_date===cell.dataset.date)return;const duplicate=assignments.some(a=>String(a.id)!==String(assignment.id)&&String(a.roster_row_id)===String(cell.dataset.row)&&a.assignment_date===cell.dataset.date&&String(a.staff_member_id)===String(assignment.staff_member_id));if(duplicate)return alert('This employee is already assigned to the destination shift.');const targetRow=allRows.find(row=>String(row.id)===String(cell.dataset.row));const dayOnly=isDayOnlyRow(targetRow);const dayViolation=dayOnly?null:restRuleViolation(assignment.staff_member_id,cell.dataset.date,{excludeAssignmentId:assignment.id});const hourViolation=dayOnly?null:eightHourRestViolation(assignment.staff_member_id,cell.dataset.date,cell.dataset.row,{excludeAssignmentId:assignment.id});if(!confirmRuleOverrides(assignment.staff_member_id,dayViolation,hourViolation))return;await DB.update('roster_assignments',assignment.id,{roster_row_id:cell.dataset.row,assignment_date:cell.dataset.date,week_start_date:week});Shared.toast(dayViolation||hourViolation?'Rostering rule override applied.':`${assignment.staff_name||'Employee'} moved to the new shift.`);await load()};
