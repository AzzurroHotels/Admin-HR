(async()=>{
  ...
  $('publish').onclick=async()=>{
    ...
    const snapshots=availableDepartments.map(dept=>{
      const departmentStaff=staff.filter(person=>normalizeDepartment(person.primary_department||person.department)===normalizeDepartment(dept.key)).map(person=>({id:person.id,full_name:person.full_name}));
      const typeRows=allRows.filter(row=>(row.department||row.staff_type)===dept.key&&row.is_active!==false).sort((a,b)=>(a.display_order||0)-(b.display_order||0)).map((row,index)=>index===0?{...row,staff_snapshot:departmentStaff}:row);
