(async()=>{
  ...
  function restRuleViolation(staffId,candidateDate,{excludeAssignmentId='',sourceAssignments=allAssignments}={}){
    const relevant=sourceAssignments.filter(item=>String(item.staff_member_id)===String(staffId)&&String(item.id)!==String(excludeAssignmentId||'')&&item.assignment_date);
