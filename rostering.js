(async()=>{
  ...
  Shared.shell('rostering','Department Rostering','Create and manage weekly rosters for every department using drag-and-drop assignments.');
  await Auth.init();
  ...
  const cfg=()=>DEPARTMENTS.find(item=>item.key===activeDepartment)||DEPARTMENTS[0];
  let week=iso(monday(new Date())), activeDepartment='Reception', staff=[], rows=[], allRows=[], assignments=[], allAssignments=[], editingRowId='', selectedStaffId='';

  content.innerHTML=`
    <div class="roster-subtabs department-roster-tabs">${DEPARTMENTS.map((item,index)=>`<button class="btn ${index===0?'btn-primary':''}" data-roster-tab="${item.key}">${item.label}</button>`).join('')}</div>
    <div class="panel-head roster-actions" style="padding:0 0 14px;border:0">
      ...
      <div><button class="btn" id="importExcel">Import Excel</button> <button class="btn" id="manageRows">Manage Roles & Shifts</button> <button class="btn" id="copy">Copy All to Next Week</button> <button class="btn btn-primary" id="publish">Publish All Rosters</button></div>
    </div>
    ...
    <div class="roster-wrap"><aside class="panel staff-panel"><h3 id="staffHeading"></h3><div id="staffList"></div></aside><div class="table-wrap"><table class="roster-table"><thead id="rh"></thead><tbody id="rb"></tbody></table></div></div>
    ...
    <div id="excelImportModal" class="modal hidden"><div class="modal-card"><div class="modal-head"><h3>Import Roster from Excel</h3><button class="btn" id="closeExcelImport">×</button></div><div class="modal-body">
      <div class="grid2"><div class="field"><label>Target Department</label><select id="excelDepartment">${DEPARTMENTS.map(item=>`<option value="${item.key}">${item.label}</option>`).join('')}</select></div><div class="field"><label>Target Week</label><input id="excelWeek" type="date"></div></div>
  ...
  async function load(){const loaded=await Promise.all([DB.list('staff_members'),DB.list('roster_rows'),DB.list('roster_assignments',`week_start_date=${week}`),DB.list('roster_assignments')]);staff=loaded[0].filter(person=>(person.account_status||person.status||'Active')==='Active');allRows=loaded[1];assignments=loaded[2].filter(item=>(item.staff_status||'Active')==='Active'&&(!item.staff_department||(item.department||item.staff_type)===item.staff_department));allAssignments=loaded[3];rows=allRows.filter(row=>row.is_active!==false);render()}
  ...
  async function importExcelRoster(){
    ...
    const staffMap=new Map(currentStaff.filter(item=>(item.primary_department||item.department)===department).map(item=>[String(item.full_name||'').trim().toLowerCase(),item]));
  ...
  $('importExcel').onclick=()=>{$('excelDepartment').value=activeDepartment;$('excelWeek').value=week;$('excelFile').value='';$('excelSheet').innerHTML='<option value="">Choose an Excel file first</option>';excelWorkbook=null;$('excelImportResult').classList.add('hidden');Shared.openModal('excelImportModal')};
  ...
  $('manageRows').onclick=async()=>{clearRow();await renderManager();$('rowModal').classList.remove('hidden')};$('closeRows').onclick=()=>$('rowModal').classList.add('hidden');$('clearRow').onclick=clearRow;$('addProperty').onclick=clearRow;
  ...
  $('publish').onclick=async()=>{
    ...
    const snapshots=DEPARTMENTS.map(dept=>{
    ...
    Shared.toast('All department rosters published');
  };
