(async()=>{
  ...
  async function load(){const loaded=await Promise.all([DB.list('staff_members'),DB.list('roster_rows'),DB.list('roster_assignments',`week_start_date=${week}`),DB.list('roster_assignments')]);staff=loaded[0].filter(isActiveStaff);allRows=loaded[1];assignments=loaded[2].filter(item=>(item.staff_status||'Active')==='Active'&&(!item.staff_department||(item.department||item.staff_type)===item.staff_department));allAssignments=loaded[3];rows=allRows.filter(row=>row.is_active!==false);render()}
  ...
  function render(){
    ...
    const people=staff.filter(s=>normalizeDepartment(s.primary_department||s.department)===normalizeDepartment(activeDepartment));
