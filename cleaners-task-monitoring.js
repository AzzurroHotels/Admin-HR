const CLEANER_SCHEDULES = [
  ...
  {
    property: 'Potts Point', shift: 'Night Shift · 6:00 PM–9:00 PM', sections: [
      ['6:00 PM–6:30 PM', ['Touch up the kitchens at 141 and 143.', 'Touch up the dining areas.', 'Clean the entrances.']],
      ['6:30 PM–7:00 PM', ['Touch up all shared bathrooms.', 'Refill bathroom supplies.', 'Remove bathroom rubbish.']],
      ['7:00 PM–7:30 PM', ['Prepare and serve dinner.', 'Keep the dining area clean while dinner is being served.']],
      ['7:30 PM–8:30 PM', ['Vacuum all corridors.', 'Vacuum the reception area.', 'Clean the courtyard and backyard.', 'Assist with guest or reception requests.']],
      ['8:30 PM–9:00 PM', ['Clean the kitchen and dining area after dinner.', 'Put the rubbish bins outside.', 'Check stickers, markers, containers, and guest supplies.', 'Report low-stock items immediately.', 'Conduct a final inspection.']]
    ]
  },
  {
  ...
  {
    property: 'Central', shift: 'Night Shift · 6:00 PM–9:00 PM', sections: [
      ['6:00 PM–6:30 PM', ['Touch up the kitchen.', 'Touch up the dining area.', 'Touch up all shared bathrooms.']],
      ['6:30 PM–7:15 PM', ['Prepare and serve dinner.', 'Keep the dining area clean while dinner is being served.']],
      ['7:15 PM–8:30 PM', ['Vacuum all corridors.', 'Vacuum the reception area.', 'Assist with guest or reception requests.']],
      ['8:30 PM–9:00 PM', ['Clean the kitchen and dining area after dinner.', 'Put the rubbish bins outside.', 'Check stickers, markers, containers, and guest supplies.', 'Report low-stock items immediately.', 'Conduct a final inspection.']]
    ]
  },
  {
...
(async function initCleanerMonitoring(){
  ...
  content.innerHTML = `
    <div class="cleaner-toolbar">
      ...
      <div class="field cleaner-property-field"><label>Property</label><div id="propertyTabs" class="cleaner-property-tabs" role="tablist" aria-label="Property filter"><button type="button" class="cleaner-property-tab active" data-property="all" role="tab" aria-selected="true">All</button>${properties.map(p=>`<button type="button" class="cleaner-property-tab" data-property="${Shared.esc(p)}" role="tab" aria-selected="false">${Shared.esc(p==='Allen / Darling Harbour'?'Allen':p)}</button>`).join('')}</div></div>
  ...
  const propertyTabButtons=[...propertyTabsEl.querySelectorAll('.cleaner-property-tab')];
  ...
  const visibleSchedules=()=>CLEANER_SCHEDULES.map((s,i)=>({...s,index:i})).filter(s=>activeProperty==='all'||s.property===activeProperty);
  const setSync=(message,type='')=>{syncEl.textContent=message;syncEl.className=`cleaner-sync-status ${type}`.trim();};
  const setBusy=busy=>{dateEl.disabled=busy;propertyTabButtons.forEach(button=>button.disabled=busy);resetBtn.disabled=busy;exportBtn.disabled=busy;};
  ...
  function render(){
    ...
    listEl.innerHTML=visible.map(schedule=>{
      ...
      return `<section class="cleaner-property-card">
        ...
        <div class="cleaner-timeline-grid">${schedule.sections.map((section,si)=>{
          const sectionDone=section[1].filter((_,ti)=>state[taskId(schedule.index,si,ti)]).length;
          const sectionPercent=section[1].length?Math.round(sectionDone/section[1].length*100):0;
          const sectionComplete=sectionDone===section[1].length;
          const sectionPartial=sectionDone>0&&!sectionComplete;
          return `<article class="cleaner-time-card ${sectionComplete?'complete':''}">
            <div class="cleaner-time-card-head">
              <h3>${Shared.esc(section[0])}</h3>
              <div class="cleaner-time-card-actions">
                <label class="cleaner-section-toggle" title="Check or uncheck all tasks in this time card" aria-label="Check or uncheck all tasks in ${Shared.esc(section[0])}">
                  <input type="checkbox" data-section-schedule="${schedule.index}" data-section-index="${si}" ${sectionComplete?'checked':''} ${sectionPartial?'data-indeterminate="true"':''}>
                </label>
                <span>${sectionDone}/${section[1].length}</span>
              </div>
            </div>
            <div class="cleaner-time-card-progress"><span style="width:${sectionPercent}%"></span></div>
            <div class="cleaner-task-list">${section[1].map((task,ti)=>{const id=taskId(schedule.index,si,ti);return `<label class="cleaner-task ${state[id]?'completed':''}"><input type="checkbox" data-task-id="${id}" ${state[id]?'checked':''}><span>${Shared.esc(task)}</span></label>`}).join('')}</div>
          </article>`;
        }).join('')}</div>
      </section>`;
    }).join('');
    ...
    listEl.querySelectorAll('input[data-task-id]').forEach(box=>box.onchange=()=>saveTask(box));
    listEl.querySelectorAll('input[data-section-schedule]').forEach(box=>{
      if(box.dataset.indeterminate==='true') box.indeterminate=true;
      box.onchange=()=>saveSection(box);
    });
  }

  async function saveSection(box){
    const scheduleIndex=Number(box.dataset.sectionSchedule);
    const sectionIndex=Number(box.dataset.sectionIndex);
    const schedule=CLEANER_SCHEDULES[scheduleIndex];
    const section=schedule?.sections?.[sectionIndex];
    if(!schedule||!section) return;

    const checked=box.checked;
    const ids=section[1].map((_,taskIndex)=>taskId(scheduleIndex,sectionIndex,taskIndex));
    const previous=Object.fromEntries(ids.map(id=>[id,state[id]===true]));
    ids.forEach(id=>{state[id]=checked;});
    render();
    setSync(checked?'Checking all tasks in this time card…':'Unchecking all tasks in this time card…','loading');

    try{
      const now=new Date().toISOString();
      const userName=Auth.user?.full_name||Auth.user?.email||'CBIT user';
      const payloads=section[1].map((description,taskIndex)=>({
        task_date:dateEl.value,
        property_name:schedule.property,
        shift_name:schedule.shift,
        task_key:ids[taskIndex],
      ...
      const {data,error}=await DB.client.from(table).upsert(payloads,{onConflict:'task_date,property_name,shift_name,task_key'}).select('id,task_key,is_completed,completed_by_name,completed_at');
      if(error) throw error;
      for(const row of data||[]){
        recordsByKey.set(row.task_key,row);
        state[row.task_key]=row.is_completed===true;
      }
      render();
      setSync('Time card saved in Supabase.','success');
    }catch(error){
      console.error(error);
      ids.forEach(id=>{state[id]=previous[id];});
      render();
      setSync(`Save failed: ${error.message}`,'error');
      Shared.toast(`Could not update the time card: ${error.message}`);
    }
  }
  ...
  function exportWord(){
    ...
    const body=visible.map(schedule=>`<h1>${Shared.esc(schedule.property)}</h1><p><b>${Shared.esc(schedule.shift)}</b></p>${schedule.sections.map((section,si)=>`<h2>${Shared.esc(section[0])}</h2>${section[1].map((task,ti)=>{const id=taskId(schedule.index,si,ti);const record=recordsByKey.get(id);const audit=state[id]&&record?.completed_at?` <span style="color:#587064;font-size:9pt">(Ticked at ${new Date(record.completed_at).toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'})}${record.completed_by_name?` by ${Shared.esc(record.completed_by_name)}`:''})</span>`:'';return `<p>${state[id]?'☒':'☐'} ${Shared.esc(task)}${audit}</p>`}).join('')}`).join('')}`).join('<hr>');
    ...
    link.download=`Cleaner_Task_Monitoring_${dateEl.value}_${activeProperty==='all'?'All_Properties':activeProperty.replace(/[^a-z0-9]+/gi,'_')}.doc`;
  ...
  propertyTabButtons.forEach(button=>{button.onclick=()=>{activeProperty=button.dataset.property;propertyTabButtons.forEach(tab=>{const isActive=tab===button;tab.classList.toggle('active',isActive);tab.setAttribute('aria-selected',String(isActive));});render();};});
  await loadState();
})();
