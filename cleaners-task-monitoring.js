    ...
    listEl.innerHTML=visible.map(schedule=>{
      ...
      return `<section class="cleaner-property-card">
        ...
        <div class="cleaner-timeline-grid">${schedule.sections.map((section,si)=>{
          ...
          return `<article class="cleaner-time-card ${sectionDone===section[1].length?'complete':''}">
            <div class="cleaner-time-card-head"><h3>${Shared.esc(section[0])}</h3><span>${sectionDone}/${section[1].length}</span></div>
