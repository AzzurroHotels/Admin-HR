(function () {
  ...
  async function enrichAssignments(rows) {
    ...
    const [staff, rosterRows] = await Promise.all([
      requireData(sb.from('staff_members').select('id,full_name,primary_department'), 'Could not load staff.'),
    ...
    return rows.map(item => {
      ...
      return { ...item, staff_name: person.full_name || '', staff_type: row.staff_type || person.primary_department || '', shift_hours: row.shift_hours || 0, paid_hours: row.paid_hours || 0 };
    });
  }
  ...
  const EvidenceStore = {
    async save(file) {
      ...
      const result = await response.json(); if (!response.ok || result.error) throw new Error(result.error || 'Upload failed.'); return result;
    },
    async remove(id) { const result = await invoke(config.functions.evidence, { body: { action: 'delete', path: id } }); return result; },
    async signedUrl(id, download = false) { const result = await invoke(config.functions.evidence, { body: { action: 'signed-url', path: id, download } }); return result.url; },
