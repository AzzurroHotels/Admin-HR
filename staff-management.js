...
const STRICT_DEDUCTION_BY_SEVERITY = { Minor: 5, Standard: 10, Major: 20, Critical: 40 };
...
async function renderStaffPage(cfg){
    ...
    const sections=visibleDepartments.map(department=>{
      ...
      const statusBlocks=statuses.map(statusName=>{
        ...
        const rowsHtml=people.length?people.map(x=>{
          ...
          const receptionRecord=ev?`<strong>${Number(ev.total_score).toFixed(1)}%</strong><div class="muted">${Shared.esc(ev.rating)}</div>${x.latest_note?`<div class="latest-note-inline"><strong>${Shared.esc(x.latest_note_type||'Record')}:</strong> ${Shared.esc(x.latest_note)}</div>`:''}`:latestRecord;
  ...
  function currentFaults(){return RECEPTION_KPI_CRITERIA.map(c=>{const count=Math.max(0,Number(qs(`[data-fault-count="${c.key}"]`)?.value||0));const note=qs(`[data-fault-note="${c.key}"]`)?.value.trim()||'';const attachments=criterionAttachments[c.key]||[];const deduction=STRICT_DEDUCTION_BY_SEVERITY[c.severity]||0;return {...c,deduction,fault_count:count,notes:note,attachments,total_deduction:count*deduction}}).filter(f=>f.fault_count||f.notes||(f.attachments&&f.attachments.length))}
  function updateScore(){
    ...
    RECEPTION_KPI_CRITERIA.forEach(c=>{
      ...
      if(el){const count=Number(qs(`[data-fault-count="${c.key}"]`)?.value||0);el.textContent=`${count*(STRICT_DEDUCTION_BY_SEVERITY[c.severity]||0)}%`}
    });
  ...
  evaluationForm?.addEventListener('submit',async e=>{
    ...
    try{
      ...
      const criticalWithoutFile=faults.filter(f=>Number(f.fault_count||0)>0&&f.severity==='Critical'&&!(f.attachments||[]).length);
      ...
      if(criticalWithoutFile.length)throw new Error('Attach at least one photo or video for every Critical fault.');
