...
async function renderStaffPage(cfg){
    ...
    const sections=visibleDepartments.map(department=>{
      ...
      const statusBlocks=statuses.map(statusName=>{
        ...
        const rowsHtml=people.length?people.map(x=>{
          ...
          const statusControl=isReadOnlyManager
          ...
          return `<tr><td><button class="link-btn" data-open="${x.id}">${Shared.esc(x.full_name)}</button></td><td>${Shared.esc(x.position_title||'-')}</td><td>${Shared.esc(x.contact_number||'-')}</td><td>${Shared.esc(x.email_address||'-')}</td><td>${Shared.esc(x.employment_type||'-')}</td><td>${Shared.fmtDate(x.date_started)}</td><td>${isReception?receptionRecord:latestRecord}</td><td>${statusControl}</td><td><button class="btn" data-open="${x.id}">${isReadOnlyManager?'View Profile':'Open Profile'}</button></td></tr>`;
      ...
      return `<section class="staff-department-section"><div class="staff-department-head"><div><h2>${Shared.esc(department)}</h2><p>${departmentPeople.length} employee${departmentPeople.length===1?'':'s'}</p></div>${isReadOnlyManager?'':`<button type="button" class="btn department-add" data-add-department="${Shared.esc(department)}">+ Add to ${Shared.esc(department)}</button>`}</div>${statusBlocks||'<div class="staff-empty-state">No employee records match the selected filters in this department.</div>'}</section>`;
    }).join('');
  ...
  staffForm.addEventListener('submit', async event => {
    ...
    if(isReadOnlyManager){Shared.toast('Manager access is read-only for employee records.');return;}
  ...
  addNote.onclick=async()=>{
    if(isReadOnlyManager)return Shared.toast('Manager access is read-only for employee records.');
