(async () => {
  ...
  document.body.innerHTML = `<div class="app public-app">
    ...
    <section class="panel public-cleaner-tasks-panel"><div class="panel-head"><div><h2>Cleaner Task Lists</h2><p class="muted">Daily cleaning schedules by property. Use the copy button to paste a property list into the housekeeping group chat.</p></div></div><div class="panel-body"><div id="publicCleanerTaskLists" class="public-cleaner-task-lists"></div></div></section>
    </main></div>`;
  ...
  function renderCleanerTaskLists(){
    ...
    host.innerHTML=CLEANER_TASK_LISTS.map((property,index)=>`<section class="public-cleaner-property">
    ...
    host.querySelectorAll('[data-copy-property]').forEach(button=>button.addEventListener('click',async()=>{
      const property=CLEANER_TASK_LISTS[Number(button.dataset.copyProperty)];
      try{
