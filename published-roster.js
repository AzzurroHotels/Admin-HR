(async () => {
  ...
  const staffSnapshot = rows => rows.find(row=>Array.isArray(row.staff_snapshot))?.staff_snapshot || [];
  ...
  function render(){
    ...
    const dates=datesForWeek(selectedWeek),rows=publication.rows||[],assignments=publication.assignments||[],people=staffSnapshot(rows);
