(async () => {
  ...
  function populateWeeks(){const weeks=availableWeeks();if(!selectedWeek||!weeks.includes(selectedWeek))selectedWeek=weeks[0]||'';$('weekSelect').innerHTML=weeks.length?weeks.map(week=>{const end=new Date(`${week}T00:00:00`);end.setDate(end.getDate()+6);return `<option value="${week}" ${week===selectedWeek?'selected':''}>${formatDate(week)} – ${formatDate(iso(end))}</option>`}).join(''):'<option value="">No published weeks</option>';$('weekSelect').disabled=!weeks.length;}
  function render(){populateWeeks();const publication=publishedRosters.find(item=>item.week_start_date===selectedWeek&&item.roster_type===activeType);if(!publication){$('publicationMeta').innerHTML='';$('publicHead').innerHTML='';$('publicBody').innerHTML='';$('publicEmpty').classList.remove('hidden');$('publicEmpty').innerHTML=`<h3>No ${esc(labelFor(activeType).toLowerCase())} has been published for this week.</h3>`;return;}$('publicEmpty').classList.add('hidden');const dates=datesForWeek(selectedWeek),rows=publication.rows||[],assignments=publication.assignments||[];$('publicationMeta').innerHTML=`<strong>${esc(labelFor(activeType))}</strong><span>${formatDate(selectedWeek)} – ${formatDate(iso(dates[6]))}</span><span>Published ${new Date(publication.published_at).toLocaleString('en-AU')}</span>`;$('publicHead').innerHTML=`<tr><th>Property / Role</th><th>Shift</th>${dates.map((date,index)=>`<th>${dayNames[index]}<br>${date.toLocaleDateString('en-AU',{day:'2-digit',month:'short'})}</th>`).join('')}</tr>`;$('publicBody').innerHTML=rows.map(row=>`<tr><td><strong>${esc(row.property_name||row.row_name||'')}</strong></td><td>${esc(row.shift_label||'')}<div class="muted">${esc(row.start_time||'')}–${esc(row.end_time||'')}</div></td>${dates.map((date,dayIndex)=>{const dateValue=iso(date),activeDays=Array.isArray(row.active_days)?row.active_days:[0,1,2,3,4,5,6];if(!activeDays.includes(dayIndex))return '<td class="roster-cell inactive-cell">—</td>';const dayAssignments=assignments.filter(item=>String(item.roster_row_id)===String(row.id)&&item.assignment_date===dateValue);return `<td class="roster-cell">${dayAssignments.map(item=>`<span class="assignment public-assignment">${esc(item.staff_name||'')}</span>`).join('')}</td>`}).join('')}</tr>`).join('')||'<tr><td colspan="9">No published roster rows.</td></tr>';}
  async function load(){try{publishedRosters=await DB.list('public/published-rosters');render()}catch(error){$('publicEmpty').classList.remove('hidden');$('publicEmpty').innerHTML=`<h3>Published roster is currently unavailable.</h3><p>${esc(error.message||'')}</p>`}}
