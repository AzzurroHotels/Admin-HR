const Auth = {
  ...
  homePage(user = this.user) {
    return user?.role === 'Manager' ? 'staff.html' : 'dashboard.html';
  },

  allowedPages(user = this.user) {
    if (!user) return ['index.html', 'published-roster.html'];
    if (user.role === 'Admin') return ['dashboard.html', 'staff.html', 'rostering.html', 'published-roster.html', 'user-management.html'];
    if (user.role === 'Manager') return ['staff.html', 'published-roster.html'];
  ...
  can(permission) {
    ...
    const managerPermissions = ['staff.view', 'evaluation.create', 'evaluation.view', 'evaluation.export'];
    return this.user.role === 'Manager' && managerPermissions.includes(permission);
  }
};
